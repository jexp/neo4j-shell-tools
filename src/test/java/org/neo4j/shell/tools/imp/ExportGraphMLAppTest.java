...
public class ExportGraphMLAppTest {
    ...
    public void testExportTinyGraph() throws Exception {
        ...
        String fileContent = new Scanner(new File("target/test.xml")).useDelimiter("\\Z").next(); assertEquals(TEST_XML_HEADER+TEST_XML_GRAPH+TEST_XML_DATA+TEST_XML_FOOTER,fileContent); } @Test public void testExportTinyGraphWithCypher() throws Exception { createData(1); assertCommand(client, "export-graphml -o target/test.xml match(n) return n", "Wrote to GraphML-file target/test.xml 0. 100%: nodes = 1 rels = 0 properties = 1 time"); String fileContent = new Scanner(new File("target/test.xml")).useDelimiter("\\Z").next(); assertEquals(TEST_XML_HEADER+TEST_XML_GRAPH+TEST_XML_DATA_NODE+TEST_XML_FOOTER,fileContent); } @Test public void testExportTinyGraphWithCypherAndRelsInBetween() throws Exception { createData(1); assertCommand(client, "export-graphml -r -o target/test.xml match (n)-[r]->() return n,r", "Wrote to GraphML-file target/test.xml 0. 100%: nodes = 1 rels = 1 properties = 2 time"); String fileContent = new Scanner(new File("target/test.xml")).useDelimiter("\\Z").next(); assertEquals(TEST_XML_HEADER+TEST_XML_GRAPH+TEST_XML_DATA+TEST_XML_FOOTER,fileContent);
    }
    @Test
    ...
    public void testExportTinyGraphWithKeys() throws Exception {
        ...
        assertCommand(client, "export-graphml -t -o target/test.xml",
                "Wrotex to GraphML-file target/test.xml 0. 100%: nodes = 1 rels = 1 properties = 2 time");
        String fileContent = new Scanner(new File("target/test.xml")).useDelimiter("\\Z").next();
        assertEquals(TEST_XML_HEADER+TEST_XML_KEYS+TEST_XML_GRAPH+TEST_XML_DATA+TEST_XML_FOOTER,fileContent);
    }
    ...
    private void createData(int nodes) {
        try (Transaction tx = db.beginTx()) {
            ...
            for (int i=0;i< nodes;i++) {
        assertEquals(TEST_XML_HEADER+TEST_XML_KEYS+TEST_XML_GRAPH+TEST_XML_DATA+TEST_XML_FOOTER,fileContent); Node to = db.getNodeById(random.nextInt(nodes));
