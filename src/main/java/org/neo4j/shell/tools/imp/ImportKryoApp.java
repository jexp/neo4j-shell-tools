...
public class ImportKryoApp extends AbstractApp {
  ...
  public Continuation execute(AppCommandParser parser, Session session, Output out)
    ...
    if (input != null) {
      out.println(String.format("Binary import file %s rel-type %s batch-size %d use disk-cache %s",
          fileName, relType, batchSize, diskSpillCache));
      ProgressReporter reporter = new ProgressReporter(kryoInputWrapper, out);
      GraphDatabaseService db = getServer().getDb();
      NodeCache cache = diskSpillCache ?
          MapNodeCache.<Long, Long>usingMapDb() : MapNodeCache.<Long, Long>usingHashMap();
