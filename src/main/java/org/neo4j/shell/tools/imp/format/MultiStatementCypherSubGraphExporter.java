...
public class MultiStatementCypherSubGraphExporter {
    ...
    public void export(PrintWriter out, int batchSize, ProgressReporter reporter) {
        boolean hasNodes = hasData(graph.getNodes());
        if (hasNodes) {
            begin(out);
            appendNodes(out, batchSize, reporter);
            commit(out);
            reporter.progress("Wrote Nodes");
        }
        ...
        if (hasData(graph.getRelationships())) {
            ...
            commit(out);
            reporter.progress("Wrote Relationships");
        }
    ...
    private long appendRelationships(PrintWriter out, int batchSize, ProgressReporter reporter) {
        long count=0;
    ...
    private void appendRelationship(PrintWriter out, Relationship rel, ProgressReporter reporter) {
        out.print("MATCH ");
        ...
        int props = formatProperties(out, rel, null);
        out.println("]->(n2);");
    ...
    private long appendNodes(PrintWriter out, int batchSize, ProgressReporter reporter) {
        long count = 0;
    ...
    private void appendNode(PrintWriter out, Node node, ProgressReporter reporter) {
        out.print("CREATE (");
        String labels = labelString(node);
        if (!labels.isEmpty()) {
            out.print(labels);
        }
        Long id = labels.startsWith(label(UNIQUE_ID_LABEL)) ? node.getId() : null;
        int props = formatProperties(out, node, id);
        out.println(");");
        reporter.update(1, props, 0);
    }

    private int formatProperties(PrintWriter out, PropertyContainer pc, Long id) {
